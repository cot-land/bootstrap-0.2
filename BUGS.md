...
### BUG-020: Large Struct Argument Passing Segfault (ARM64)

**Status:** FIXED (2026-01-28)
**Priority:** P0 (was blocking bootstrap tests)
**Discovered:** 2026-01-28
**Location:** `test/bootstrap/all_tests.cot:1962`

**Symptoms:**
Passing a large struct (>16 bytes) as a function argument caused a segfault on ARM64. The crash occurred when calling `sum_big_arg(s)` where `BigArg` is a 48-byte struct (6 x i64 fields).

**Root Cause:**
When expand_calls rewrote the call to pass a pointer instead of loading the struct value, the original load operation was marked as dead (uses=0). However, regalloc then created a store_reg to spill the dead load, which incremented uses back to 1. At codegen time, the now-non-dead load was generated, but its source address wasn't properly materialized (came out as 0x0), causing the null pointer crash.

**Fix:**
Added a two-pass dead code elimination in codegen (`src/codegen/arm64.zig` and `src/codegen/amd64.zig`):
1. First pass: iterate backwards through values, propagating dead uses to arguments
2. Second pass: skip dead values when generating code

Also ensured `load_reg` is never skipped as it's needed to restore spilled values.

**Files Changed:**
- `src/codegen/arm64.zig:838-877` - Added dead code elimination before codegen
- `src/codegen/amd64.zig:1420-1459` - Same fix for AMD64
